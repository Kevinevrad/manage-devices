...
import { affectationStatuts, getAffectationStatut } from "@/data/affectations";

import { affectationColumns } from "./affectations-columns";
import type { AffectationsTableProps } from "./affectations-table.types";
    ...
    return data.filter((affectation) => {
      const matchQuery =
        ...
        [
          ...
          affectation.numSerie,
          affectation.utilisateur,
          affectation.service,
        ].some((value) => value.toLowerCase().includes(query));
      const matchStatut =
        statut === "tous" || getAffectationStatut(affectation) === statut;
      const matchService =
        service === "tous" || affectation.service === service;
      return matchQuery && matchStatut && matchService;
    });
  }, [data, search, statut, service]);
  ...
  return (
    <Card className="gap-0 py-0">
      ...
      <div className="flex flex-col gap-3 p-5 lg:flex-row lg:items-center lg:justify-between">
        <div className="space-y-0.5">
          <h2 className="font-heading text-lg font-semibold tracking-tight">
            Affectations du parc
          </h2>
          <p className="text-sm text-muted-foreground">
            Qui détient quel matériel : remises, retours planifiés et
            restitutions.
          </p>
        </div>
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="Rechercher (équipement, utilisateur, service…)"
        ...
        <TableBody>
          {table.getRowModel().rows.length ? (
            ...
            <TableRow className="hover:bg-transparent">
              <TableCell
                colSpan={affectationColumns.length}
      ...
      <div className="flex flex-col gap-3 border-t px-5 py-3 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-muted-foreground">
          {rowCount} affectation{rowCount > 1 ? "s" : ""} — page{" "}
